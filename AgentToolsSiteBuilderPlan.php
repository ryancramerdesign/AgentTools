...
Each field has name, disposition (create/reuse/update), type, label, summary, and settings. Each template has name, disposition, label, summary, dataOnly, singleton, fields[], allowedParents, allowedChildren, and settings. Each template field has name, required, columnWidth, and settings. Each page has key, disposition, parent (page key or null), name, template, status, values, and optional contentBrief. Each file has path, role, disposition, summary, optional template, and optional methods. File roles template and pageClass both require template. Modules are explicit objects with name, source, and disposition. Verification contains routes[], adminPages[], and checks[].
	...
	The request supplies a color scheme and may supply a brand color. Choose a coherent palette suited to the subject and audience when the scheme is auto, and build around the brand color when one is supplied unless the scheme is monochrome. Unless the selected scheme is monochrome, the primary accent must be a real hue rather than black, white, or grey. Tint neutral backgrounds, surfaces, and borders slightly toward the palette instead of relying on pure greys. Check text and accent contrast against their backgrounds in both light and dark palettes. Record the complete palette in design.tokens so the user can review it before approval.
