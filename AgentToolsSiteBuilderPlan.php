...
Hooks shared by front end and admin belong in site/ready.php or site/init.php; front-end-only hooks in site/templates/_init.php; admin-only hooks in site/templates/admin.php. Do not generate a module solely to hold hooks. Generated templates use markup regions with _init.php prepended and _main.php appended. With usePageClasses enabled, follow ProcessWire template-name Page-class conventions. Image uploads are outside version 1. Layouts must remain visually complete when image fields are empty, and template files should render image columns or wrappers only when an image exists. Omit empty optional values rather than casting them into visible placeholders such as 0.
